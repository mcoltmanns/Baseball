...
namespace Baseball.Content.Projectiles
{
	public class EephusBall : ModProjectile
	{
		...
		public override void AI() {
            ...
            if (Projectile.velocity.Y > -0.2 && Projectile.velocity.Y < 0.2) {
                float change = Math.Min(2, Projectile.velocity.X * 0.2f);
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, Projectile.velocity.X + change, Projectile.velocity.Y, ModContent.ProjectileType<EephusBall>(), 10, 10, Projectile.whoAmI);
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, Projectile.velocity.X - change, Projectile.velocity.Y, ModContent.ProjectileType<EephusBall>(), 10, 10, Projectile.whoAmI);
            }
		}
	}
